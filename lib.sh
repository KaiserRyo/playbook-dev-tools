...
function bootstrap()
{
  ALLPROGS="nettle gmp gnutls taskwarrior flac flac123 nss rpm alpine ruby git bash-static libgcrypt tgl libmpdclient mpd cronie protobuf khal protobuf mosh weechat SDL bash-static libgpg-error  sqlite libuuid taskwarrior openssl nettle libtasn1 gnutls syncthing gmp libffi nettle coreutils gettext perl vim seafile libreadline libevent zeromq openssh coreutils grep gzip bzip2 jansson libmpg123 gdb xz cflow libevent gcc diffutils make patch bison gettext findutils groff man file tg ffmpeg audioplayer $NEWPROGS"
